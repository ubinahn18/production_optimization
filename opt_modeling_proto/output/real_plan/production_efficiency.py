...
전체를 놓고 다음 지표를 계산한다:
  ...
  - 용기생산량/튜브생산량/마스크생산량: 그 제품군 라인(_data_source.
    build_line_category_lookup으로 판단 - product_id가 아니라 라인
    기준, "코드확인중" 같은 placeholder product_id의 category 혼동을
    피하기 위함)에서 계획기간 전체 동안 실제로 생산(produce)된 수량의
    합. 슬롯 하나 = 1시간이므로 슬롯당 rate(그 라인/주문의 시간당
    생산량, _data_source.build_rate_lookup) 그대로가 그 슬롯의 생산량.
...
from _data_source import build_line_category_lookup, build_rate_lookup
...
def compute_efficiency(
    ...
    produce["category"] = produce["line_id"].map(_category)
    ...
    mask_qty = qty_by_category.get("마스크", 0.0)
    ...
    return {
        ...
        "tube_qty": tube_qty,
        "mask_qty": mask_qty,
        "other_qty": sum(v for k, v in qty_by_category.items() if k not in ("용기", "튜브", "마스크")),
...
def main():
    ...
    r = compute_efficiency(schedule, daily_workforce, rate_lookup, line_category)

    print(f"용기 생산량: {r['container_qty']:,.0f}개")
    print(f"튜브 생산량: {r['tube_qty']:,.0f}개")
    print(f"마스크 생산량: {r['mask_qty']:,.0f}개")
    if r["other_qty"]:
