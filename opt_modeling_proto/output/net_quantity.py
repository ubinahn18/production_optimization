...
주문의 category(_data_source.build_order_category_lookup)로 묶어서
제품군별 합계를 낸다.
...
이 때문에 잘못된 결과가 나온 적 있음). category도 product_id가 아니라
order_id로 판단한다 - 예전엔 물리 line_id 기준으로 판단했었지만(라인은
category 전용이라는 가정), 한 물리 라인(예: "로타리")이 "마스크"와
"마스크_멀티시트"처럼 서로 다른 category의 주문을 같이 생산할 수 있게
되면서 그 가정이 깨졌다 - order_id는 라인과 무관하게 항상 유일하고
category도 주문마다 명확하므로 이 문제가 생기지 않는다.

카테고리 이름은 하드코딩하지 않고 실제 Order 데이터에 있는 값을 그대로
쓴다 - --real-plan(엑셀 기반)에서는 "마스크"/"마스크_멀티시트"/"용기"/
"튜브"가 나오고, 내장 예시 데이터에서는 "mask"/"container"/"tube"가 나온다.
...
from _data_source import add_source_args, build_order_category_lookup, build_rate_lookup, resolve_source
...
def compute_net_quantity(
    ...
    order_id 기준으로 정한다(위 모듈 docstring 참고). rate/category 정보가
    ...
    day_df["category"] = day_df["order_id"].map(_category)
...
def main():
    ...
    by_order = compute_net_quantity(schedule, args.day, rate_lookup, order_category)
    by_category = (
