...
동일 물리 라인이 여러 대(k대) 있으면(예: line_container_1가 19대), k대
사이에는 symmetry가 있다 - 겹치지 않는 시간대의 작업을 어느 물리 카피에
배정하든 비용/연속성 목적함수 값은 완전히 동일하다. 라인마다 변수를 따로
만드는 방식으로는 CP-SAT이 이 symmetry(최대 k! 개의 동등한 배정) 때문에
"이게 최적이다"를 증명하는 데 막대한 시간을 쓰게 된다. 그래서 그룹 크기가
config.pooling_min_group_size보다 큰 그룹은 라인별 변수 대신 "이 슬롯에
이 그룹에서 몇 대가 무슨 상태인가"라는 집계 정수 변수로 모델링해서
symmetry를 원천적으로 없앤다(물리 라인 식별자 자체가 모델에 안 나옴).
작은 그룹(기본 3대 이하, 지금 내장 예시 데이터의 최대 그룹 크기)은 기존
라인별 방식을 그대로 써서 예시 데이터의 결과를 그대로 보존한다. 풀링된
그룹의 해를 사람이 보는 물리 라인별 스케줄로 되돌리는 건 CP-SAT과 무관한
순수 후처리이며 scheduling/pooling.py에 있다.
"""
...
def build_and_solve(lines: list[Line], orders: list[Order], config: ScheduleConfig) -> ScheduleResult:
    ...
    # ------------------------------------------------------------------
    # 동일 물리 라인 그룹 탐지 + legacy(라인별)/pooled(집계) 분리.
    #   그룹 크기(k)가 config.pooling_min_group_size 이하면 기존 라인별
    #   변수 방식을 그대로 쓴다(작은 그룹은 symmetry 손해가 미미하고,
    #   내장 예시 데이터의 결과를 그대로 보존하기 위함). 그보다 큰 그룹만
    #   풀링(집계 카운트) 방식으로 모델링한다.
    # ------------------------------------------------------------------
    detected_pools = detect_line_pools(lines, orders)
    pooled_pools: list[LinePool] = [p for p in detected_pools if p.k > config.pooling_min_group_size]
    pooled_member_ids = {lid for p in pooled_pools for lid in p.member_line_ids}
    line_to_pool: dict[str, LinePool] = {lid: p for p in pooled_pools for lid in p.member_line_ids}

    def compat_pools_for(compat_line_ids: list[str]) -> list[LinePool]:
        """주문의 호환 라인 목록(compatible_lines())에서, 풀링 대상인
        물리 라인들이 속한 풀들을 중복 없이 뽑는다."""
    ...
    line_ids = [l.line_id for l in lines if l.line_id not in pooled_member_ids]
    ...
    #   하나의 활동만 수행" 제약). [이 블록은 legacy 라인에만 적용된다 -
    #   pooled 라인은 이 아래 별도 블록에서 집계 변수로 다룬다.]
    # ------------------------------------------------------------------
    ...
    # 하단의 model.AddHint 호출 참고). 이름은 all_bool_vars지만 아래 풀링
    # 블록의 IntVar(0..k)도 그대로 담는다 - AddHint는 IntVar 전반에 동일하게
    # 동작한다.
    ...
    # ------------------------------------------------------------------
    # 풀링된 라인 그룹(config.pooling_min_group_size보다 큰 동일 라인 그룹)의
    # 집계 변수 + 제약. 개별 물리 라인을 구분하지 않고 "이 슬롯에 이 그룹의
    # 몇 대가 무슨 상태인가"만 정수 카운트(0..k)로 추적한다.
    #
    ...
    #                            (2단계 연속성 목적함수에서 씀)
    #
    # 각 변수의 의미와 day-boundary(fresh 강제 리셋)에도 불구하고 전환
    # 횟수 집계가 정확한 이유는 설계 논의(계획 문서) 참고 - 핵심은 풀링된
    # 유닛들이 완전히 상호교환 가능해서, 어느 순간의 유휴/가동 총량 차이가
    # 곧 그 순간의 실제(달성 가능한 최소) 전환 횟수와 정확히 같다는 것.
    # ------------------------------------------------------------------
    ...
    for p in pooled_pools:
        ...
        # 셋업 -> 다음 슬롯 반드시 그 제품 생산(하드 제약, 라인별 모델의
        # `is_prod[t+1]==1 OnlyEnforceIf(su)`와 동급). t, t+1 양쪽 변수가
        # 다 만들어진 뒤에 걸어야 하므로 별도 루프로 뺀다.
        for t in range(T - 1):
            for oid in p.compat_order_ids:
    ...
    for o in orders:
        ...
        terms = []
        for lid in compat:
            if lid in pooled_member_ids:
                continue  # 풀링된 라인은 아래에서 풀 단위로 따로 집계
    ...
    #   빼고 생산(legacy는 is_prod, pooled는 prod_fresh+prod_nf)만
    #   인원 수요에 반영한다. (참고: 야간 셋업은 애초에 셋업 슬롯 자체가
    #   생기지 않으므로 - 위 전이 제약 참고 - 별도 처리가 필요 없다.)
    # ------------------------------------------------------------------
    max_possible_workers = sum(
        o.workers.get(lid, 0) for lid in line_ids for o in compat_orders_by_line[lid]
    ) + sum(
        p.workers.get(oid, 0) * p.k for p in pooled_pools for oid in p.compat_order_ids
    )
    ...
    #     풀링된 그룹도 같은 논리로 |n_idle_pool[t]-n_idle_pool[t-1]|을
    #     쓴다 - 풀링된 유닛들은 완전히 상호교환 가능해서, 이 값이 곧
    #     그 순간 실제로 달성 가능한 최소 전환 횟수와 정확히 같다
    #     (day-boundary도 예외 없이 동일한 식으로 처리된다).
    #   - 셋업 횟수: is_setup(legacy) + pool_setup(pooled) 변수를 전부
    #     더한 값 = 전체 기간 동안 발생한 제품 전환(셋업) 총 횟수. 셋업(Y)
    #     다음 슬롯은 항상 그 제품(Y) 생산으로 이어지도록 하드 제약을
    #     걸어뒀기 때문에, 이 값은 정확히 '실제로 발생한 제품 전환
    #     이벤트 수'와 같다. 다만 대기<->생산 전환처럼 셋업도 "생산 중이던
    #     라인이 하던 일을 멈추고, 다른 걸 준비해서, 다시 생산으로
    #     돌아오는" 동일한 성격의 사건이므로(대기가 '들어감+나옴' 2점인
    #     것과 대칭적으로), 셋업 1회도 2점으로 계산한다(가중치 2배). 그래서
    ...
    if config.optimize_continuity:
        ...
        # ------------------------------------------------------------------
        # 3차 타이브레이크: 풀링된 그룹에서 "새로 fresh 유닛을 쓰는 것"
        # (prod_fresh)을 최소화한다.
        #
        ...
        tiebreak_scale = sum(p.k for p in pooled_pools) * T + 1 if pooled_pools else 1

        model.Minimize(
            (
                sum(switch_terms) + 2 * sum(setup_terms)
                + sum(pool_switch_terms) + 2 * sum(pool_setup_terms)
            ) * tiebreak_scale
